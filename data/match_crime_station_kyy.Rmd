...
##x<-read.csv("data/subwaycrime_with_station_new.csv")
kmeans_df<-read_csv("data/Kmeans_centers.csv")
...
```{r}
crime_df<-read_csv('data/subway.csv') %>% 
  janitor::clean_names() %>% 
  mutate(cmplnt_fr_tm=as.character(cmplnt_fr_tm),
         cmplnt_to_tm=as.character(cmplnt_to_tm))
crime_df2<-read_csv('data/subway_newest.csv') %>% 
  janitor::clean_names() %>% 
  mutate(cmplnt_fr_tm=as.character(cmplnt_fr_tm),
         cmplnt_to_tm=as.character(cmplnt_to_tm))%>% 
  select(-new_georeferenced_column) 
...
for (each in 1:nrow(crime_df_total)){
 ...
 crime_df_total[,42]= a%>% pull(service)
 crime_df_total[,43]= a%>% pull(administrative_area_level_1)
crime_df_total %>% 
  distinct(cluster)

# bigger_distance<-crime_df_total %>% 
...
```{r}
kmeans_df_compare<-function(each){
  closet_row<-
    kmeans_df %>% 
    mutate(lat_new=crime_df_total[each,]$latitude,
         long_new=crime_df_total[each,]$longitude) %>% 
...
for (each in 1:nrow(crime_df_total)){
  b<-b %>% 
    bind_rows(kmeans_df_compare(each))
...
crime_df_total<-crime_df_total %>% 
  mutate(cluster=b$cluster) %>% 
  mutate()

crime_df_ny<-crime_df_total %>% 
...
crime_df_new <-
  crime_df_total %>% 
    mutate(station=closest_station,
           closest_linename=closest_line,
