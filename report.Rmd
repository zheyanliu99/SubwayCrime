...
There are two tab designed in the on the shiny dashboard([click]("https://stephenyuan.shinyapps.io/CrimeMapShiny/") here). The first tab shows crime data, which includes cluster map and bar charts by the service and cluster. In this tab, the users can control the crime type and time period to check the plots in different scenarios. The second crime shows the crime rate mainly, which contains zipcode map and bar charts by the service and cluster. Users can choose the time period detailed to days and type of the crime's degree. 
