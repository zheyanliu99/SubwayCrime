...
![Wordcloud using victims description](images/Word_Cloud.png){width=65%}
...
The records of some original data is not quite reasonable, for instance, the proceeding time of some violation or misdemeanor cases last for more than several years, it is not clear whether that case really takes such a long period or not, so that leads to extremely unreasonable outliers. Second, There would be some censoring data in the original data set for specific year, for example, only several days in January and February of 2020 were recorded in the data set, so it may become hard to explore the true situation regarding crime events in this period of time. 
